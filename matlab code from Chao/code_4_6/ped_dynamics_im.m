...
%-------------------------------------------------------------------------%
function f0a = ped_dynamics_im(t,xi,ur,q)
global m bdry p p1 p2 D  BDRY 
global tau v0a0 v0ai vmax r A1a A1aa A2a B1a B2a lambda A1 B1 VS dist 
global nr rr 
...
%% --------------- Pedestrian Interactions Force (normal)--------------- %%
%     fab = [0,0];
%     for j = 1:m
%         if (i ~= j)
%             dab = norm(x(i,:) - x(j,:));
...
%------------------ interaction force model 2 (panic)---------------------%
%         fab = [0,0];
%         %k = 1.2*10^2; kappa = 2.4*10^2;
%         k = 20; kappa = 100;
%         for j = 1:m
%             if (i ~= j)
%                 dab = norm(x(i,:) - x(j,:));
...
%                          (lambda + (1 - lambda)*((1+(-nab*e(i,:)'))/2))+...
%                          k*gab)*nab + kappa*gab*(v(j,:) - v(i,:))*tab'*tab;
%     
%                 end
%             end
%         end
...
%         f((2*i)) = f((2*i)) + fab(1,2);
%         
% ------------- interaction force model 3 (crowd pressure)----------------%
%       fab = [0,0];
%       k = 2; D0 = 0.31; D1 = 0.45;
%       for j = 1:m
%           if (i~=j)
...
%                   rab = r(i) + r(j);
%                   nab = ((x(i,:) - x(j,:))/dab);
%                   fab = fab + A1aa*exp(( - dab)/D0 + (D1/dab)^k)*nab*...
%                         (lambda + (1 - lambda)*((1+(-nab*e(i,:)'))/2));
%               end
%           end
%       end
...
% -------------- interaction force model 3 (combined)---------------------%
      fab = [0,0];
      k = 2; kappa = 2000; D0 = 0.31; D1 = 0.45;
      for j = 1:m
          if (i~=j)
              dab = norm(x(i,:) - x(j,:));
              if (dab < VS)
    ...
    if (nr ~=0)
        ...
        for j = 1:nr
            ...
            if (dar < VS)
                rar = r(i) + rr(j);
                nar = ((x(i,:) - xr(j,:))/dar);
                far = far + ((A1a*exp((rar - dar)/B1a))*nar);%...
 %                   *(lambda1 + (1 - lambda1)*((1+(-nar*e(i,:)'))/2));
 %                   + ((A2a*exp((rar - dar)/B2a))*nar);
            end
        end
